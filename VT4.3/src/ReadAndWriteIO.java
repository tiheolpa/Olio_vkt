	...
	public void readAndWrite(String path, String outputFile) {
		try {
			...
			while ((inputline = in.readLine()) != null) {
				...
				if ((inputline.isEmpty() != true) && (inputline.length() < 30) && (inputline.contains("v") == true)) { // Samanlainen kirjoitusmenetelmä kuin edellisessä, nyt on vain ehto kirjoittamiselle
					out.write(inputline + "\n");
				}
			}
			in.close();
